...
async def creatgr():
    ...
    desc = "Group Log untuk Zar Userbot.\n\nJANGAN KELUAR LO MONYED NANTI CRASH!.\n\n⛧ Powered By ~ @uTits ⛧"
    try:
        grup = await bot(
            CreateChannelRequest(title="ZAR UBOT LOGS", about=desc, megagroup=True)
        )
...
async def autobot():
    ...
    if isdone.startswith("Sorry,"):
        ...
        if nowdone.startswith("Done!"):
            ...
            await bot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await bot.send_file(bf, "rams/utils/styles/zarsist.jpg")
            await asyncio.sleep(3)
            await bot.send_message(bf, "/setabouttext")
            await asyncio.sleep(1)
            await bot.send_message(bf, f"@{username}")
            await asyncio.sleep(1)
            await bot.send_message(bf, f"⛧ Owner bot: {who.first_name}")
            await asyncio.sleep(3)
